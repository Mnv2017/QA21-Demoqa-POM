...
public class SelectMenuPage extends PageBase {
    ...
    public SelectMenuPage clickOnOldStyle(String text) {
        ...
        select.selectByVisibleText(text); //выбор варианта из списка

        List<WebElement> options = select.getOptions(); //возвращает список параметров объекта select
    ...
    @FindBy(xpath = "//html") // локатор "пустого поля"
    WebElement space;
    ...
    public SelectMenuPage clickStandardMultiSelect() {
        ...
        if (multi.isMultiple()) { // если в списке несколько элементов
            multi.selectByIndex(1); // выбираем по индексу
            multi.selectByIndex(2);
            multi.selectByIndex(3);
        }
        List<WebElement> selectOpt = multi.getOptions();
